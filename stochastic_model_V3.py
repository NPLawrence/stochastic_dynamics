...
class MDN_module(nn.Module):
    ...
    def __init__(self, fhat, V = None, n = None, beta = 0.99, k = 1, is_training = True, show_mu = False, mode = None):
        super().__init__()
    ...
    def forward(self, x, y = None):
        ...
        # print(var)
        # comp = MultivariateNormal(mu_stable, torch.diag_embed(var))
        # print('hello')
        # mix = D.Categorical(torch.ones(x.shape[0],1))
        # comp = D.Independent(D.Normal(mu_stable.view(-1,1,2), var.view(-1,1,2)),1)
        # print(comp)
        # model_dist = MixtureSameFamily(mix, comp)
        model_dist = MultivariateNormal(mu_stable, torch.diag_embed(var))
        fx = (model_dist.rsample())

        if self.is_training:
            logp_y = -(model_dist.log_prob(y)).mean()
            # print(logp_y)
            # print(-(model_dist1.log_prob(y).squeeze()).mean())
            return logp_y
        else:
